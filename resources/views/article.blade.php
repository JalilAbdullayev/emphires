...
@section('image', asset('storage/blog/' . $article->image))
@section('content')
    <x-child-header :bg="$article->background ? asset('storage/blog/' . $article->background) : null" :title="__('Blog')" :subtitle="$article->title" :route="'blog_' . session('locale')" />
    <main class="container grid py-20 lg:grid-cols-12">
        <section class="px-[15px] max-md:pt-20 max-lg:pb-[50px] lg:col-span-9">
            <div class="relative">
                <img src="{{ asset('storage/blog/' . $article->image) }}" alt="" class="w-full img-fluid" />
            </div>
            <article class="p-[30px]">
                <div class="article-meta">
                    <span>
                        <i class="fa-solid fa-folder-open"></i> <a
                            href="{{ route('blog_category_' . session('locale'), $article->category->slug) }}">
                ...
                <div class="flex justify-between items-center border-t border-[#EEEEEE] mt-[30px] pt-5">
                    <span class="mr-5 capitalize">
                        <span class="text-base font-semibold text-[#424D6C]">
                            Tags: <span class="font-normal text-[#8B90A0]">
                            {{ $article->keywords }}
                            </span>
                        </span>
                    </span>
        ...
        <aside class="lg:col-span-3 pl-[30px]">
            <form action="{{ route('blog_search_' . session('locale')) }}"
                ...
                <ul>
                    @foreach ($categories as $category)
                        <li class="relative p-0 my-0 5">
                            <a href="{{ route('blog_category_' . session('locale'), $category->slug) }}">
