...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <!-- Tab panes -->
            <form action="{{ route('admin.whyus.update', $reason->id) }}" method="POST">
                @csrf
                        ...
                        <div @class(['tab-pane', 'active' => $loop->first]) id="{{ $language }}" role="tabpanel">
                            <div class="form-floating my-3">
                                <input class="form-control" name="title_{{ $language }}" id="title"
                                    ...
                                    value="{{ $reason->getTranslation('title', $language) }}" required />
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control ckeditor" placeholder="@lang('Description')" id="description"
                                    name="description_{{ $language }}">{{ $reason->getTranslation('description', $language) }}</textarea>
                            </div>
                        </div>
                    @endforeach
                </div>
                <div class="form-floating my-3">
                    <input class="form-control" name="icon" id="icon" placeholder="@lang('Icon')"
                        type="text" maxlength="255" value="{{ $reason->icon }}" />
